...
`
	...
	tidbDMTemplate string = `#!/bin/bash
{{.Copyright}}
# Generated by dbdeployer {{.AppVersion}} using {{.TemplateName}} on {{.DateTime}}
source {{.SandboxDir}}/sb_include
...
# From template
TIDB_PORT="{{.Port}}"
DM_MASTER_PORT="1{{.Port}}"
DM_WORKER_PORT="2{{.Port}}"
...
var TidbTemplates = TemplateCollection{
	...
	"tidb_dm_template": TemplateDesc{
		Description: "helper file to setup Data Migration (DM) for TiDB",
		Notes:       "",
		Contents:    tidbDMTemplate,
	},
}
